...
import { X, User, Calendar, Paperclip, Send, Play, Pause, Check, Save, GripVertical } from 'lucide-react'
import { cn } from '@/lib/utils'
import type { GdqTaskCard, GdqTaskStatus } from '@/types/gdq-plan'
import type { TaskMessage } from '@/types/marketing-plan'
import { GDQ_STAGES, GDQ_STATUS_LABEL } from '@/lib/gdq-status'

interface TaskModalProps {
  task: GdqTaskCard
  isNew?: boolean
  onClose: () => void
  onUpdate: (id: string, patch: Partial<GdqTaskCard>) => void
  onCreate?: (task: GdqTaskCard) => void
...
export function TaskModal({ task, isNew, onClose, onUpdate, onCreate, onSendMessage, onSave }: TaskModalProps) {
  ...
  const patch = (p: Partial<GdqTaskCard>) => {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => patch({ status: e.target.value as GdqTaskStatus })}
                className="w-full rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-700 focus:border-obi-300 focus:outline-none"
              >
                {GDQ_STAGES.map((s) => (
                  <option key={s.id} value={s.id}>
                    {GDQ_STATUS_LABEL[s.id]}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="mt-auto pt-5">
            {isNew ? (
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(current.status === 'nova-tarefa' || current.status === 'pendente') && (
                  <button
                    type="button"
                    onClick={() => patch({ status: 'em-execucao' })}
