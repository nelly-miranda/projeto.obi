...
import type { GdqStage, GdqTaskCard } from '@/types/gdq-plan'
...
interface TaskColumnProps {
  ...
  onDragEndTask: () => void
  onDropTask: (stageId: GdqStage['id']) => void
  onRequestCreate: (stageId: GdqStage['id']) => void
}
    ...
    <div className="flex h-full w-[280px] shrink-0 flex-col">
      <div className="rounded-t-2xl bg-slate-900 px-3.5 py-2.5">
        <p className="truncate text-[13px] font-semibold text-white">
          {stage.label} <span className="text-slate-400">{tasks.length}</span>
        </p>
      </div>
      ...
      >
        ...
        <button
          type="button"
          onClick={() => onRequestCreate(stage.id)}
